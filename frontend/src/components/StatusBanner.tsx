    ...
    <section className={`panel border-l-4 p-5 ${blocked ? "border-l-risk" : "border-l-accent"}`}>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-6">
        ...
        <div>
          ...
          <p className="mt-1 text-base font-semibold text-ink">{transactionCount || 128}</p>
        </div>
      </div>
      <p className="mt-3 text-sm leading-6 text-slate-600">
        {blockingIssues
          ? `F5 computation can be reviewed, but final approval is blocked until unresolved high-severity anomalies are cleared. ${highSeverityAnomalies || 2} high-severity anomalies require accountant action.`
          : period?.status === "APPROVED"
            ? "Approved for manual submission via IRAS myTax Portal. This prototype does not submit directly to IRAS."
            : "AI has completed initial classification. Human review is required for 6 transactions. No automatic IRAS submission will occur."}
      </p>
    </section>
  );
}
